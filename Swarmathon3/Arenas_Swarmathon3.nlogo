...
to move-to-location
  ...
  ifelse (pxcor = locX and pycor = locY)[
       ...
       ask patch-here[ set pclor baseColor ]
...
This is a model to model deterministic searching, which differs from the biological searching methods we have used in previously.

## HOW IT WORKS

There is a setup button which clears all values as well as the screen, sliders that can 
